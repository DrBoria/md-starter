/**
 * Transforms a URL path into a human-readable format.
 *
 * @param path - The URL path to transform.
 * @returns A human-readable string derived from the path.
 */
function transformPathToReadableFormat(path: string): string {
  ...
  const readableSegments = segments.map((segment, index) => {
    ...
    const words = cleanSegment.split("-").map((word) => {
...
export { transformPathToReadableFormat };
