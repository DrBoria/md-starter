...
const useFieldsData = <T extends FlexibleItemData>({
  ...
  if (fieldsToRender?.length) {
    ...
    if (!requestedFields.includes("name")) requestedFields.push("name"); // NOTE: name is required field
  ...
  const { update, loading } = useUpdateMutation(list, selectedFields);

  // memoize the data fetching operation depending on itemId
  const fetchData = useCallback(() => {
    return itemId
      ? useQueryListItem<DeepNullable<T>>({ listName, selectedFields, itemId })
      : useQueryList<DeepNullable<T>>({
          listName,
          selectedFields,
          limit: limit || 10,
          skip,
